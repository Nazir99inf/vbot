module.exports = async (m, { conn, isROwner, isOwner, isMods, isPrems, isBotAdmin, isAdmin, isRAdmin, groupMetadata }) => {
    ...
    if (m.mtype === "interactiveResponseMessage" && m.msg?.nativeFlowResponseMessage?.paramsJson) {
        try {
            const fakeMsg = {
                ...
                message: {
                    conversation: JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id
                },
                messageTimestamp: Math.floor(Date.now() / 1000),
                pushName: m.pushName || m.name || "User",
                participant: m.sender,
    ...
    if (m.mtype === "templateButtonReplyMessage" && m.quoted?.fromMe && m.msg?.selectedId) {
        conn.appendTextMessage?.(m, m.msg.selectedId, chatUpdate);
    }

    if (m.mtype === "buttonsResponseMessage" && m.quoted?.fromMe && m.msg?.selectedButtonId) {
        conn.appendTextMessage?.(m, m.msg.selectedButtonId, chatUpdate);
    }
};
