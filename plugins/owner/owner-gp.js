...
${ListPlugins.map((a, i) => `*${i + 1}.* ${a}`).join("\n")}`;
        m.reply(wait);
        if (isNaN(text)) {
            if (!fs.existsSync(process.cwd() + "/plugins/" + text))
                throw `*• Example :* ${usedPrefix + command} *[filename]*

* *List Plugins ( ${ListPlugins.length} Files ) :*
${ListPlugins.map((a, i) => `*${i + 1}.* ${a}`).join("\n")}`;
            m.reply(fs.readFileSync(process.cwd() + "/plugins/" + text).toString());
        } else {
            if (!fs.existsSync(process.cwd() + "/plugins/" + ListPlugins[text - 1]))
                throw `*• Example :* ${usedPrefix + command} *[filename]*
