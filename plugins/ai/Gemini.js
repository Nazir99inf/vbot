...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) throw `*• Example :* ${usedPrefix + command} Apa Itu AI`;
  try {
    await m.react("⏳");
    let res = await gemini({ message: text })
    await m.react("✅");
    await conn.sendMessage(m.chat, {
      ...
      contextInfo: {
        externalAdReply: {
          ...
          thumbnailUrl: "https://i.pinimg.com/736x/a8/5e/00/a85e000378e6650e9de84358ab06b780.jpg",
    ...
    console.error(e);
    throw "*Gagal mendapatkan respon dari Gemini.*";
  }
};

handler.help = ["gemini", "gm"].map((a) => a + " [pertanyaan atau balas media]");
handler.tags = ["ai"];
