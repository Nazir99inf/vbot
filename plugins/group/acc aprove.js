let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    switch (subCommand) {
        case "list":
            const formattedList = joinRequestList.length > 0 ? joinRequestList.map((request, i) => `*${i + 1}.*\n• Nomor: ${request.jid.split("@")[0]}\n• Metode Permintaan: ${request.request_method}\n• Waktu Permintaan: ${formatDate(request.request_time)}\n\n`).join("") : "Tidak ada permintaan bergabung yang tertunda.";
