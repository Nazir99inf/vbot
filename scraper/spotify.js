...
class Parser {
    ...
    parseSearch(res) {
        ...
        const parse = (arr, mapFn, isTrack = false) =>
            (arr || []).reduce((acc, node) => {
        ...
        return {
            ...
            albums: parse(res.albumsV2?.items, a => ({
                ...
                color_dark: this._getCol(a.coverArt),
                visual_identity: this._getVI(a.visualIdentity)
            })),
            artists: parse(res.artists?.items, art => ({
                ...
                color_dark: this._getCol(art.visuals?.avatarImage),
                visual_identity: this._getVI(art.visualIdentity)
            })),
            episodes: parse(res.episodes?.items, ep => ({
                ...this._getLink(ep.uri),
                name: ep.name || null,
                description: ep.description || null,
                duration_ms: ep.duration?.totalMilliseconds || 0,
                explicit: ep.contentRating?.label === "EXPLICIT",
                media_types: ep.mediaTypes || [],
                release_date: ep.releaseDate?.isoString || null,
                playability: { playable: ep.playability?.reason === "PLAYABLE", reason: ep.playability?.reason || null },
                played_state: ep.playedState?.state || null,
                is_paywall: !!ep.restrictions?.paywallContent,
                images: this._getImg(ep.coverArt),
                color_dark: this._getCol(ep.coverArt),
                visual_identity: this._getVI(ep.visualIdentity),
                video_preview_thumbnail: this._getImg(ep.videoPreviewThumbnail?.imagePreview?.data),
            ...
            podcasts: parse(res.podcasts?.items, pod => ({
                ...this._getLink(pod.uri),
                name: pod.name || null,
                publisher: pod.publisher?.name || null,
                media_type: pod.mediaType || null,
                topics: (pod.topics?.items || []).map(t => ({ ...this._getLink(t.uri), uri: t.uri, title: t.title })),
                images: this._getImg(pod.coverArt),
                color_dark: this._getCol(pod.coverArt),
                visual_identity: this._getVI(pod.visualIdentity)
            })),
            playlists: parse(res.playlists?.items, pl => ({
                ...
                color_dark: this._getCol(pl.images?.items?.[0]),
                visual_identity: this._getVI(pl.visualIdentity),
            ...
            genres: parse(res.genres?.items, g => ({
                ...this._getLink(g.uri),
                name: g.name || null,
                images: this._getImg(g.image),
                color_dark: this._getCol(g.image)
            })),
            users: parse(res.users?.items, u => ({
                ...this._getLink(u.uri),
                display_name: u.displayName || null,
                username: u.username || null,
                images: this._getImg(u.avatar),
                color_dark: this._getCol(u.avatar)
            }))
        };
    }
    ...
    parseArtist(data) {
        ...
        return {
            ...this._getLink(a.uri || `spotify:artist:${a.id}`),
            uri: a.uri || `spotify:artist:${a.id}`,
            name: a.profile?.name || null,
            verified: !!a.profile?.verified,
            images: this._getImg(a.visuals?.avatarImage),
            header_images: this._getImg(a.visuals?.headerImage?.data || a.headerImage?.data),
            color: this._getCol(a.visuals?.avatarImage),
            ...
            top_tracks: (a.discography?.topTracks?.items || []).map(node => ({
                ...this._getLink(node.track?.uri),
                name: node.track?.name || null,
                playcount: parseInt(node.track?.playcount) || 0,
                duration_ms: node.track?.duration?.totalMilliseconds || 0,
    ...
    parseAlbum(data) {
        ...
        return {
            ...this._getLink(al.uri),
            name: al.name || null,
            type: al.type || null,
            release_date: al.date?.isoString || al.date?.year || null,
            label: al.label || null,
            playability: { playable: !!al.playability?.playable, reason: al.playability?.reason || null },
            images: this._getImg(al.coverArt),
            color: this._getCol(al.coverArt),
            visual_identity: this._getVI(al.visualIdentity),
    ...
    parsePlaylist(data) {
        ...
        return {
            ...
            images: this._getImg(pl.images?.items?.[0] || pl.image),
            color: this._getCol(pl.images?.items?.[0] || pl.image),
            visual_identity: this._getVI(pl.visualIdentity),
            owner: {
                ...this._getLink(pl.ownerV2?.data?.uri),
                display_name: pl.ownerV2?.data?.name || null,
                username: pl.ownerV2?.data?.username || null,
                images: this._getImg(pl.ownerV2?.data?.avatar)
            },
            tracks: (pl.content?.items || pl.tracks?.items || [])
                .map(node => {
...
class Spotify {
    ...
    async download(url) {
        try {
            if (!url?.includes("sp")) throw "Error Gada Link Nya!!";
            ...
            const cookie = req.headers["set-cookie"].map(v => v.split(";")[0]).join("; ");
            ...
            const k = await sock.post(
                "/getTrackData",
                qs.stringify({
                    spotify_url: url
                })
            );
            const d = await sock.post(
                "/convert",
                qs.stringify({
                    urls: url
                })
            ...
            const m = k.data;
            m.download = d.data;
            if (!m?.download) throw "Semua Metadata Nya Error!!";
            return {
                id: m.id,
                type: m.type,
                name: m.name,
